...
AZURE_OPENAI_API_VERSION = "2024-10-01-preview" # or "2025-08-28"
...
class OpenAIRTHandler():
    ...
    def __init__(self) -> None:
        ...
        self.connection_manager = self.client.beta.realtime.connect(
                    model="gpt-realtime"  # Replace with your deployed realtime model id on Azure OpenAI.
            )
...
async def test_openai_versions():
    async with OpenAIRTHandler() as handler:
        while True:
            user_input = input("Enter a message (q to quit): ")
            if user_input.lower() == "q":
                break
            await handler.send_message(user_input)
